    ...
    <script>
        ...
        function renderProductTable(filter = '') {
            ...
            filtered.forEach(p => {
                const tr = document.createElement('tr');
                tr.innerHTML =
                    `<td>${p.name}</td><td style='text-align:right;'>₱${parseFloat(p.price).toFixed(2)}</td><td style='text-align:center;'>${p.quantity}</td><td style='text-align:center;'><input type='checkbox' class='product-select-checkbox' data-id='${p.id}'></td>`;
                tbody.appendChild(tr);
            });
        }
