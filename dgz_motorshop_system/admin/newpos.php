...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/newpos.css">
</head>
...
document.getElementById('addSelectedProducts').onclick = function () {
    ...
    (function(){
      ...
      function computeChange(){
        ...
        if (btn) { btn.disabled = (total <= 0 || received < total); }
      }
      ...
      document.addEventListener('DOMContentLoaded', function(){
        ...
        if (table) {
          table.addEventListener('input', function(e){
            if (e.target && (e.target.matches('.pos-qty') || e.target.matches('input[type="number"]'))) { recalcTotal(); }
          });
          table.addEventListener('change', function(e){
            if (e.target && e.target.matches('input[type="checkbox"]')) { recalcTotal(); }
          });
        }
        var clearBtn = document.getElementById('clearPosTable');
        if (clearBtn) clearBtn.addEventListener('click', function(){ setTimeout(recalcTotal, 0); });
